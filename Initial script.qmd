...
# Export the necessary functions and variables to the cluster nodes
clusterExport(cl, c("fetch_openalex_data", "reconstruct_abstract", "extract_openalex_data"))

# Progress bar wrapper with parallel processing
fetch_openalex_data_with_progress <- function(dois, email) {
  pb <- txtProgressBar(min = 0, max = length(dois), style = 3)
  
  fetched_data <- pblapply(dois, function(doi) {
    res <- fetch_openalex_data(doi, email = email)
    if (getTxtProgressBar(pb) %% 1000 == 0) {
...
# Fetch data with progress
batch_size <- 1000
num_batches <- ceiling(length(altmetrics_data$DOI) / batch_size)
fetched_data_list <- list()

for (i in 1:num_batches) {
  start_idx <- (i - 1) * batch_size + 1
  end_idx <- min(i * batch_size, length(altmetrics_data$DOI))
  batch_dois <- altmetrics_data$DOI[start_idx:end_idx]
  
  fetched_data_batch <- fetch_openalex_data_with_progress(batch_dois, email = "a.watt2@lse.ac.uk")
  fetched_data_list[[i]] <- rbindlist(lapply(fetched_data_batch, extract_openalex_data), fill = TRUE)
}

# Combine all batches into a single data table
fetched_data_dt <- rbindlist(fetched_data_list, fill = TRUE)
